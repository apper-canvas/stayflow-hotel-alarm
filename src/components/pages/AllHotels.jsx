...
const AllHotels = () => {
  ...
  const loadAllHotels = async () => {
    try {
      ...
      const results = await hotelService.getAll()
  ...
  const handleFiltersChange = (filters) => {
    ...
    if (filters.priceRange[1] < 1000) {
      filtered = filtered.filter(hotel => hotel.pricePerNight >= filters.priceRange[0] && hotel.pricePerNight <= filters.priceRange[1])
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        {error ? (
          <Error message={error} onRetry={loadAllHotels} />
        ) : (
            ...
            <main className="flex-1">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-gray-600">{filteredHotels.length} hotels available</p>
                </div>
